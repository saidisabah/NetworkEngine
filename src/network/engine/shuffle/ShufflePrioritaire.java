...
public class ShufflePrioritaire {

    private static final int CONTROL_PORT = 9610;
    private static final int REQUEST_PORT = 9210;
    private static final int DATA_PORT = 9310;
    private static final int FIN_PORT = 9910;
    private static final int BLOCK_SIZE = 250 * 1024 * 1024; // 250 Mo
...
System.out.println("===========================================");
        ...
        try (ServerSocket serverSocket = new ServerSocket(CONTROL_PORT)) {
            ...
            while (true) {
                try (Socket socket = serverSocket.accept();
                    ...
                    Map<String, Integer> plan = (Map<String, Integer>) in.readObject();
 
                    System.out.println("🔑 Rôle reçu : " + role);
 
                    if (role.startsWith("sender")) {
                        System.out.println("🧾 Je suis " + role + ", je dois envoyer :");
                        for (Map.Entry<String, Integer> entry : plan.entrySet()) {
                            System.out.println("➡️  " + entry.getValue() + " blocs à " + entry.getKey());
                        }
                        startRequestListener();
 
                    } else if (role.startsWith("receiver")) {
                        System.out.println("🧾 Je suis " + role + ", je vais recevoir :");
 
                        int total = 0;
                        for (Map.Entry<String, Integer> entry : plan.entrySet()) {
                            System.out.println("⬅️  " + entry.getValue() + " blocs de " + entry.getKey());
                            total += entry.getValue();
                        }
                        System.out.println("📦 Total attendu : " + total + " blocs");
 
                        startDataReceiverServer(total);
 
                        List<String> senderOrder = loadSenderOrder(SENDER_COUNT);
                        Queue<String> receptionQueue = buildPriorityQueue(plan, senderOrder);
                        
                        System.out.println("📜 File de réception prioritaire (ordre prévu) :");

                        int index = 1;
                        for (String sender : receptionQueue) {
                            System.out.println(" " + index + "️⃣ Bloc de " + sender);
                            index++;
                        }
 
                        sendRequestsFromPlan(receptionQueue);
                    }
    ...
    private static List<String> loadSenderOrder(int senderCount) {
        List<String> senders = new ArrayList<>();
        try (BufferedReader reader = new BufferedReader(new FileReader(WORKERS_FILE))) {
            String line;
            int count = 0;
            while ((line = reader.readLine()) != null && count < senderCount) {
                senders.add(line.trim());
                count++;
            }
        } catch (IOException e) {
            System.err.println("❌ Erreur lecture fichier workers : " + e.getMessage());
        }
        return senders;
    }
 
    private static Queue<String> buildPriorityQueue(Map<String, Integer> plan, List<String> senderOrder) {
        Map<String, Integer> remaining = new HashMap<>(plan);
        Queue<String> queue = new LinkedList<>();
    
        while (!remaining.isEmpty()) {
            // Liste des candidats ayant le maximum de blocs restants
            int max = Collections.max(remaining.values());
            List<String> maxSenders = new ArrayList<>();
            for (String sender : senderOrder) {
                if (remaining.getOrDefault(sender, 0) == max) {
                    maxSenders.add(sender);
                }
            }
    
            // Ajout cyclique des senders avec le max de blocs
            for (String sender : maxSenders) {
                if (remaining.containsKey(sender) && remaining.get(sender) > 0) {
                    queue.add(sender);
                    remaining.put(sender, remaining.get(sender) - 1);
                    if (remaining.get(sender) == 0) {
    ...
    // Nouvelle version suggérée de sendRequestsFromPlan
private static void sendRequestsFromPlan(Queue<String> receptionQueue) {
    String myIP = getMyIPFromWorkers();
    if (myIP.isEmpty()) {
        System.err.println("❌ Impossible de récupérer mon IP depuis le fichier workers.");
        return;
    }
 
    //System.out.println("📡 IP forcée par variable d'environnement : " + myIP);
 
    int blocIndex = 1;
    sendStartToMaster();
    while (!receptionQueue.isEmpty()) {
        ExecutorService batchExecutor = Executors.newFixedThreadPool(R_MULTITHREAD);
        List<Future<Boolean>> futures = new ArrayList<>();
 
        // Créer un lot de MULTITHREAD demandes
        for (int i = 0; i < R_MULTITHREAD && !receptionQueue.isEmpty(); i++) {
            String senderIP = receptionQueue.poll();
            final int currentBloc = blocIndex;
            blocIndex++;
            ...
            futures.add(batchExecutor.submit(() -> {
                ...
                while (!success) {
                    try (Socket socket = new Socket(senderIP, REQUEST_PORT);
                        ...
                        out.writeObject(currentBloc);
                        System.out.println("📨 Demande envoyée à " + senderIP + " pour bloc " + currentBloc);
 
                        String ack = ackQueue.poll(60, TimeUnit.SECONDS);
                        if (ack != null) success = true;
                        else {
                            System.err.println("⏱️ Timeout bloc " + currentBloc + ", retry...");
                            Thread.sleep(1000);
                        }
 
                    } catch (IOException | InterruptedException e) {
                        System.err.println("❌ Erreur demande bloc " + currentBloc + " à " + senderIP + ": " + e.getMessage());
                        Thread.sleep(1000);
                    }
                }
                return true;
        ...
        for (Future<Boolean> future : futures) {
            try {
                future.get(); // attend la fin
            } catch (Exception e) {
                System.err.println("❌ Erreur lors de l'attente du batch : " + e.getMessage());
            }
        }
...
new Thread(() -> {
    while (true) {
        ...
        if (req != null) {
            ...
            try {
                Thread.sleep(50); // évite de saturer le CPU
    ...
    try (ServerSocket serverSocket = new ServerSocket(DATA_PORT)) {
        ...
        int blocCounter = 1;
        long receiverStartTime = System.currentTimeMillis(); // 🕒 Début
        ...
        while (true) {
            try (Socket socket = serverSocket.accept();
                ...
                if (totalRead > 0) {
                    ...
                    long elapsed = System.currentTimeMillis() - receiverStartTime;
                    System.out.printf("📥 Bloc %d reçu de %s (%d octets) ⏱️ [%d ms depuis le début]%n",
                        blocCounter,
